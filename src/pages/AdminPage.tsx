...
import { BarChart3, Trash2, UsersRound } from 'lucide-react';
  ...
  return (
    <section className="animate-panel-in">
      ...
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-slate-600">管理用户、对局记录和全站统计。</p>
      </div>
      ...
      <div className="panel mt-6 p-5">
        ...
        <div className="mt-4 overflow-auto">
          <table className="data-table">
            <thead><tr><th>时间</th><th>用户</th><th>模式</th><th>胜者</th><th>手数</th><th>操作</th></tr></thead>
            <tbody>
              {records.map((record) => (
                <tr key={record.id}>
                  <td>{new Date(record.createdAt).toLocaleString()}</td>
                  <td>{record.userEmail || '本地访客'}</td>
                  <td>{record.playerMode === 'ai' ? '人机' : record.playerMode === 'online' ? '在线双人' : '本地双人'}</td>
                  <td>{record.winner || '-'}</td>
